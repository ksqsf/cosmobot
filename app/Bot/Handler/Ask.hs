...
askHandlers
  :: (Chat.Chat :> es, ChatLog.ChatLog :> es, AgentAudit.AgentAudit :> es, LLM.LLM :> es, Memory.Memory :> es, Scheduler.Scheduler :> es, Log :> es, IOE :> es)
  => Agent.ToolConfig
  -> AskHandlerConfig
  -> ConversationStore
  -> [RouteHandler es]
  ...
  stopOn continuedMessage \message parentId ->
    forkEff do
      ...
      case parent of
        ...
        Just conversation ->
          continueConversation toolCfg cfg conversations message parentId conversation
  where
    continuedMessage =
