  ...
  <section class="page">
    ...
    >
      ...
      >
        ...
        >
          <div><span class="summary-mark violet"><i class="pi pi-chart-bar" /></span><span><strong>{{ inspectorStats.tokens?.total_tokens.toLocaleString() ?? 'Unreported' }}</strong><small v-if="inspectorStats.tokens">Tokens · {{ inspectorStats.tokens.prompt_tokens.toLocaleString() }} prompt / {{ inspectorStats.tokens.completion_tokens.toLocaleString() }} completion</small><small v-else>Token usage was not returned by the model provider</small><small v-if="inspectorStats.promptCacheHitRate !== null">Prompt cache · {{ (inspectorStats.promptCacheHitRate * 100).toFixed(1) }}% hit · {{ inspectorStats.cachedPromptTokens?.toLocaleString() }} cached</small><small v-else>Prompt cache · Unreported</small></span></div>
          <div><span class="summary-mark success"><i class="pi pi-arrow-right" /></span><span><strong v-if="inspectorStats.latestTokens">{{ inspectorStats.latestTokens.prompt_tokens.toLocaleString() }} input / {{ inspectorStats.latestTokens.completion_tokens.toLocaleString() }} output</strong><strong v-else>Unreported</strong><small>Latest model request</small></span></div>
