...
describe('overview derivations', () => {
  ...
  it('counts audit failures from failed tools and interrupted runs', () => {
    expect(auditFailureCount([
      audit(1, { tag: 'ToolCallFinished', runId: 'run-1', toolName: 'read', status: 'ok' }),
      audit(2, { tag: 'ToolCallFinished', runId: 'run-1', toolName: 'write', status: 'timeout' }),
  ...
  it('merges live records once, keeps the bounded order, and renders newest activity first', () => {
    const records = mergeAuditRecords([
      audit(1, { tag: 'AgentRunStarted', runId: 'run-1' }),
      audit(2, { tag: 'ModelTurnStarted', runId: 'run-1', turn: 1 }),
    ], audit(2, { tag: 'AgentRunFinished', runId: 'run-1', status: 'answered' }), 2)
