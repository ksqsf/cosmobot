...
schema = Schema.ConfigSchema
  ...
  , Schema.options =
      ...
      , Schema.optionalOption ["access_key_id"] "Access key ID" "S3 access key identifier." owner Schema.secret False Aeson.Null (fmap Schema.Secret . (.accessKeyId)) (fmap Schema.Secret . (.accessKeyId))
      , Schema.optionalOption ["secret_access_key"] "Secret access key" "S3 secret access key." owner Schema.secret False Aeson.Null (fmap Schema.Secret . (.secretAccessKey)) (fmap Schema.Secret . (.secretAccessKey))
